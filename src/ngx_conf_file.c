...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    if (filename) {
        ...
        cf->conf_file = &conf_file;
        //读取/usr/local/nginx/conf/nginx.conf获取文件描述符
        ...
        //定义解析文件的类型
        type = parse_file;
        //不影响主流程，暂定cf->conf_file->dump=NULL
        if (ngx_dump_config
#if (NGX_DEBUG)
            || 1
#endif
    ...
    for ( ;; ) {
        ...
        if (b->pos >= b->last) {
            ...
            if (len == NGX_CONF_BUFFER) {
                ...
                if (d_quoted) {//表示缺少一个双引号
                    ch = '"';

                } else if (s_quoted) {//表示缺少一个单引号
                    ch = '\'';
