...
int ngx_cdecl
main(int argc, char *const *argv)//argc——参数数量；argv——参数序列
{
    ngx_buf_t        *b;
    ngx_log_t        *log;
    ngx_uint_t        i;
    ...
    // if (ngx_strerror_init() != NGX_OK) {//错误信息相关的初始化，如果初始化失败则直接退出
    //     return 1;
    // }
    
    ngx_write_stderr("*****************************************************************************************************\n");

    if (ngx_get_options(argc, argv) != NGX_OK) {//解析命令行参数，给各个变量赋值
        return 1;
    }
    ...
    /* TODO */ ngx_max_sockets = -1;//初始化ngx_max_sockets，该变量在os/unix/ngx_posix_init.c中定义，为ngx_int_t类型
...
// #endif
    ...
    ngx_parent = ngx_getppid();//获取父进程ID
    ngx_write_stderr("*****************************************************************************************************pid\n");
    // printf(ngx_pid);

    log = ngx_log_init(ngx_prefix, ngx_error_log);//日志初始化
    if (log == NULL) {
        return 1;
    }
...
// #endif
    ...
    ngx_memzero(&init_cycle, sizeof(ngx_cycle_t));//init_cycle初始化
    init_cycle.log = log;
    ngx_cycle = &init_cycle;
    ngx_write_stderr("*****************************************************************************************************cycle\n");
    init_cycle.pool = ngx_create_pool(1024, log);//创建nginx内存池
    if (init_cycle.pool == NULL) {
        return 1;
    }
    ngx_write_stderr("*****************************************************************************************************pool\n");
    if (ngx_save_argv(&init_cycle, argc, argv) != NGX_OK) {//保存参数
        return 1;
    }
    ngx_write_stderr("*****************************************************************************************************save_arg\n");
    if (ngx_process_options(&init_cycle) != NGX_OK) {//初始化cycle的配置文件
        return 1;
    }
    ngx_write_stderr("*****************************************************************************************************process options\n");
    if (ngx_os_init(log) != NGX_OK) {//初始化nginx的系统配合
        return 1;
    }
    ngx_write_stderr("*****************************************************************************************************osinit\n");
    /*
     * ngx_crc32_table_init() requires ngx_cacheline_size set in ngx_os_init()
     */

    if (ngx_crc32_table_init() != NGX_OK) {//初始化nginx的crc32_table
        return 1;
    }
    ngx_write_stderr("*****************************************************************************************************crc\n");
    /*
     * ngx_slab_sizes_init() requires ngx_pagesize set in ngx_os_init()
     */

    ngx_slab_sizes_init();//初始化一些变量

    if (ngx_add_inherited_sockets(&init_cycle) != NGX_OK) {//继承已存在的sockets
        return 1;
    }

    if (ngx_preinit_modules() != NGX_OK) {//初始化模块的一些信息
        return 1;
    }
    ngx_write_stderr("*****************************************************************************************************preinit\n");
    ...
    //     ngx_write_stderr("******************************************************ngx_log_redirect_stderr(cycle) != NGX_OK\n");
    //     return 1;
    // }
    ngx_write_stderr("******************************************************03\n");
    ...
    if (ngx_process == NGX_PROCESS_SINGLE) {
        ngx_write_stderr("*******************************************************************ngx_process == NGX_PROCESS_SINGLE\n");
        ngx_single_process_cycle(cycle);
    } else {
        ngx_write_stderr("*******************************************************************ngx_process == NGX_PROCESS_MASTER\n");
...
ngx_get_options(int argc, char *const *argv)
{
    ...
    for (i = 1; i < argc; i++) {//直接用参数第二位开始
        ...
        while (*p) {
            ...
            case 'p':
                if (*p) {//安装路径  形如-p/usr/local/nginx/
                    ngx_prefix = p;
                    goto next;
                }

                if (argv[++i]) {//形如-p /usr/local/nginx/
            ...
            case 'c'://配置文件路径
                if (*p) {//形如-c/conf/nginx.conf
                    ngx_conf_file = p;
                    goto next;
                }

                if (argv[++i]) {//形如-c /conf/nginx.conf
            ...
            //另一个约束条件是，以-g方式启动的Nginx服务执行其他命令时，需要把-g参数也带上，否则可能出现配置项不匹配的问题
            case 'g':
                if (*p) {
                    ngx_conf_params = p;
                    goto next;
...
ngx_process_options(ngx_cycle_t *cycle)
{
    ...
    if (ngx_prefix) {
        ...
        ngx_str_set(&cycle->conf_prefix, NGX_CONF_PREFIX);//设置cycle->conf_prefix为conf、
#else
        ngx_str_set(&cycle->conf_prefix, NGX_PREFIX);
#endif
        ngx_str_set(&cycle->prefix, NGX_PREFIX);//设置cycle->prefix为/usr/local/nginx/
    ...
    if (ngx_conf_file) {
        ...
        //设置cycle->conf_file为conf/nginx.conf
    ...
    //将cycle->conf_prefix从/nginx设置为/usr/local/nginx/conf/nginx.conf
